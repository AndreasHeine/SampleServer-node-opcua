...
import { DataType, resolveNodeId, AttributeIds, getHostname } from "node-opcua";
import {
...
const createWriterGroup = (
  name: string,
  dataSetWriter: MyJsonDataSetWriterDataTypeOptions,
): MyMqttJsonWriterGroupDataTypeOptions => {
...
const createConnection = (
  ...
  const writerGroup: MyMqttJsonWriterGroupDataTypeOptions = createWriterGroup(
    writerGroupName,
    dataSetWriter,
  );
...
export const constructMqttJsonPubSubConfiguration = (broker: string) => {
  const opts: PubSubConfigurationDataTypeOptions = {
    ...
    publishedDataSets: [createPublishedDataSet1(), createPublishedDataSet2()],
  };
