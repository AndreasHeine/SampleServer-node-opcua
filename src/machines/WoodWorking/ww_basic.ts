...
export const createWoodWorkingBasicLogic = async (addressSpace: AddressSpace): Promise<void> => {
    ...
    bsProductInstanceUriNode?.setValueFromSource({ dataType: DataType.String, value: "https://www.example.com" });
    bsSerialNumberNode?.setValueFromSource({ dataType: DataType.String, value: "378485745354392" });
    bsYearOfConstructionNode?.setValueFromSource({ dataType: DataType.UInt16, value: 2023 });

    // Set basic woodworking STATE - MACHINE - FLAGS nodes
    bsAlarmNode?.setValueFromSource({ dataType: DataType.Boolean, value: false });
    bsCalibratedNode?.setValueFromSource({ dataType: DataType.Boolean, value: true });
    bsEmergencyNode?.setValueFromSource({ dataType: DataType.Boolean, value: false });
    bsMachineInitNode?.setValueFromSource({ dataType: DataType.Boolean, value: false });
    bsMachineOnNode?.setValueFromSource({ dataType: DataType.Boolean, value: true });
    bsPowerPresentNode?.setValueFromSource({ dataType: DataType.Boolean, value: true });
    bsRecipeInRunNode?.setValueFromSource({ dataType: DataType.Boolean, value: true });
    bsWarningNode?.setValueFromSource({ dataType: DataType.Boolean, value: false });
    ...
    function setBasicWwValues() {
        ...
        // Set basic woodworking STATE - MACHINE - FLAGS nodes
        bsAlarmNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm < 450 ? false : true)});
        bsCalibratedNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm > 170 ? false : true) });
        bsEmergencyNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm < 350 ? false : true) });
        bsMachineInitNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm > 150 ? false : true) });
        bsMachineOnNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm > 40 ? false : true) });
        bsPowerPresentNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm > 130 ? false : true) });
        bsRecipeInRunNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm > 200 ? false : true) });
        bsWarningNode?.setValueFromSource({ dataType: DataType.Boolean, value: (rndm < 350 ? false : true) });

        // Set current mode STATE - MACHINE - OVERWIEW
        switch (true) {
            case (rndm > 0 && rndm < 100):
                bsCurrentModeNode?.setValueFromSource({ dataType: DataType.Int32, value: 1 });
                bsCurrentStateNode?.setValueFromSource({ dataType: DataType.Int32, value: 4 });
                break;
            case (rndm > 100 && rndm < 200):
                bsCurrentModeNode?.setValueFromSource({ dataType: DataType.Int32, value: 2 });
                bsCurrentStateNode?.setValueFromSource({ dataType: DataType.Int32, value: 3 });
                break;
            case (rndm > 200 && rndm < 300):
                bsCurrentModeNode?.setValueFromSource({ dataType: DataType.Int32, value: 3 });
                bsCurrentStateNode?.setValueFromSource({ dataType: DataType.Int32, value: 2 });
                break;
            case (rndm > 300 && rndm < 400):
                bsCurrentModeNode?.setValueFromSource({ dataType: DataType.Int32, value: 4 });
                bsCurrentStateNode?.setValueFromSource({ dataType: DataType.Int32, value: 1 });
                break;
            case (rndm > 400 && rndm < 500):
                bsCurrentModeNode?.setValueFromSource({ dataType: DataType.Int32, value: 5 });
                break;
            default:
                break;
